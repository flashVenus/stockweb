			...
			<el-main class="transform-main " style="overflow: hidden;">
					...
					<div class="tab-bg" style="display: flex;justify-content: space-between; height: 40px;">
						<div style="display: flex;justify-content: space-between;">
							...
							<div class style="cursor: pointer; display: flex;">
								...
								<div class="optional-list">
									<div class="one-line-list">
										<div class="optional-item-hover" @click="optTablebox(item, index, '1', item.zi)"
											:class="currIndex == index ? 'currIndex' : ''"
											v-for="(item, index) in marketNot" :key="index">{{ item.name }}</div>
									</div>
								</div>
							</div>
							<!-- 登录后行情 -->
							...
							<div class style="margin-left: 15px;display: flex;">
								...
								<div class="optional-list">
									<div class="one-line-list">
										<div class="optional-item-hover" v-for="(item, index) in information"
											:key="index" @click="optionalTablebox(item, index, '2')"
				...
				<el-row :gutter="3">
					<el-col class="hei" :span="4" v-if="cutIndex == 2">
						<div :style="'height:' + windowHeight + 'px'" class="listhi-cont"
							...
							<div style="display: flex;justify-content: center;margin-top: 20px;"
								v-if="newsList.length <= 0">暂无数据</div>

							<div class="newsList-item"
								style="margin:0 10px 10px 10px; padding-bottom: 10px;cursor: pointer;"
								v-for="(item, index) in newsList" :key="index" @click="selectDetails(item, index)">
								<div style="display: flex;margin-top: 20px;">
									<div style="font-size: 12px;position: relative;">
										<span class="iconfont icon-triangle-left sanjiao"
											:class="index < 3 ? 'red' : 'ccc'"></span>
										<span class="quantity" :class="index < 3 ? '' : 'bg-cc'">{{ index + 1 }}</span>
									</div>
									<div style="font-weight: bolder;font-size: 12px;" class="history-title">
										{{ item.title }}</div>
								</div>
								<div
									style="display: flex; justify-content: space-between;margin-top: 12px;margin-left:28px;">
									<div>
										<span style="font-size: 12px;color:rgb(131, 131, 131);">{{ item.sourceName
										}}：</span>
										<span style="font-size: 12px;color:rgb(131, 131, 131);">{{ item.showTime
										}}</span>
									</div>
								</div>
							</div>
					...
					<el-col :span="5" v-if="cutIndex == 1">
						<!--  ==> 监听账户下单 -->
						<table-box ref="tableBox" @toTransaction="toTransaction" :hasGetNewOrder="hasGetNewOrder2"
							:handleOptions2="handleOptionsindex2" :handleOptions3="handleOptions3"
							:handleOptions4="handleOptions4"></table-box>
					</el-col>
					<el-col class="alterWidthCenter" :span="14" v-if="cutIndex == 1">
						...
						<!-- 持仓单子 -->
						<div v-if="$store.state.haslogin && xuanzhongitem == '指数'"
							style="display:flex;align-items: center;justify-content: center;margin-top: 10vh;">
							暂无数据
						</div>
						<div class="tab-box jiaoyi-000">
							<el-tabs v-model="activeNameZero" class="black-style"
								v-if="$store.state.haslogin && xuanzhongitem != '指数'">
								<el-tab-pane label="买入/卖出" name="zero">
									<buy-box1 @selectDetailsItem="selectDetailsItem" :cutIndex="cutIndex"
										:detailsCont="detailsCont" :hasGetNewOrder="hasGetNewOrder"
										:handleOptions2="handleOptions2" :settingInfo="settingInfo" :code="code">
									</buy-box1>
								</el-tab-pane>
							...
							<div v-if="false" class="account-state">
								<span
									:class="$store.state.userInfo.allProfitAndLose > 0 ? 'red' : $store.state.userInfo.allProfitAndLose < 0 ? 'green' : ''">持仓总盈亏：{{
											$store.state.userInfo.allProfitAndLose
									}}</span>
								<span style="color:#409EFF;">冻结保证金：{{ $store.state.userInfo.allFreezAmt }}</span>
								<span style="color:#d06e45;">强制平仓线：{{ ($store.state.userInfo.enableAmt +
							...
							<div class="new-list-jiaoyi" v-show="!$store.state.haslogin" style="display: flex;">
								<div class="news-cont">
									...
									<div @click="selectDetails(item, index)" v-for="(item, index) in transactionNewList"
										:key="index"
								...
								<div class="news-cont">
									...
									<div @click="selectDetails(item, index)"
										v-for="(item, index) in transactionNoticeList" :key="index"
										style="font-size: 12px;display: flex; justify-content: space-between;padding:6px 0;">
										<div class="news-list">
											<span class="red">•</span>
											<span>•</span>
