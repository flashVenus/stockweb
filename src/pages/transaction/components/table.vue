							...
							<template slot-scope="scope">
								<div class="tab-name">
									...
									<span class="code">
										...
										<i v-else-if="scope.row.stock_type === 'bj'" class="iconfont bei-stock">北</i>
										{{scope.row.code}}
									</span>
								</div>
							</template>
	...
	export default {
		...
		methods: {
			...
			async refreshList() {
				...
				let data = null;
				if (this.activeName === "first" || this.activeName === "four" || this.activeName == 'start') {
			...
			async toTransaction(row, column, event) {
				...
				// }
				let code =
					this.activeName === "first" || this.activeName === "four" || this.activeName == 'start' ?
					row.code :
					row.stockCode;
