            ...
            <el-progress :text-inside="true" :stroke-width="10"
				...
				<el-collapse v-model="accountActiveNames">
					<div class="count-r" style="display: none">
						<span class="line">
							（您的沪深账户强制卖出线为
							<span class="num">{{ (($store.state.userInfo.enableAmt + $store.state.userInfo.allFreezAmt) *
						...
						<el-col :span="24">
							<el-row class="Assets-box" :gutter="20">
								...
								<el-col :span="6">
									<div class="box box1">
										<i class="color1 iconfont icon-dongjiezijin"></i>
										<p class="title">今日浮动盈亏:</p>
										<p>
											<span
												class="number">￥{{ $store.state.hide ? '****' : $store.state.userInfo.allFloatProfitAndLose }}</span>
										</p>
									</div>
								</el-col>
								<el-col :span="6">
					...
					<div class="count-r" style="display: none">
						<span class="line">
							（您的指数账户强制卖出线为
							<span class="num">{{ (($store.state.userInfo.allIndexFreezAmt +
					...
					<div class="count-r" style="display: none">
						<span class="line">
							（您的期货账户强制卖出线为
							<span class="num">{{ (($store.state.userInfo.allFuturesFreezAmt +
									$store.state.userInfo.enableFuturesAmt)).toFixed(2)
							}}</span>
							）
						</span>
					</div>
							...
							<el-row class="Assets-box" :gutter="20">
								...
								<el-col :span="6">
									<div class="box box1">
										...
										<p>
											<span class="number">￥{{ (($store.state.userInfo.allFuturesFreezAmt +
	...
	methods: {
		...
		async drawLine() {
			...
			// 	Number(this.$store.state.userInfo.userFuturesAmt)).toFixed(2)
				var alldata = '￥' + (Number(this.$store.state.userInfo.enableAmt) + Number(this.$store.state.userInfo.allFreezAmt) + Number(this.$store.state.userInfo.newStockWaitPay)).toFixed(2)
			// console.log(data);
			...
			myChart.setOption({
				...
				series: [{
					...
					data: [
					{
						value: this.$store.state.userInfo.enableAmt,
						name: data
					},
					{
						value: this.$store.state.userInfo.allFreezAmt,
						name: data1
					},
					{
