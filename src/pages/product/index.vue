          ...
          <slot>
            <div class="bancot">
              <div class="description">
                <div class="tit"><span class="zh">产品说明</span><span class="en">PRODUCT DESCRIPTION</span></div>
                <div class="text">我们最大的资产是我们的员工，{{siteInfo.siteName}}只聘用那些富有经验的专业人员，他们已经在金融服务业展示了他们一流的客户服务技巧和领带能力。提供专业的客户服务。我们的销售和营运代表将以专业的中文对客户的任何询问进行解答。</div>
              </div>
            </div>
          </slot>
        </backdrop1>
                ...
                <div class="s_right">
                  <div class="not_l" style="width: 35px;overflow: hidden;"><span class="iconfont icon-kaihuhedui"></span>操盘须知</div>
                    ...
                    <ul>
                      ...
                      <li>当操盘资金低于卖出线下时，我们有权将您的账户实行止损操作;</li>
              ...
              <div class="info">
                <div class="left">
                  ...
                  <div class="item">
                    ...
                    <div class="msg">可买多、卖空，买升、买趺、当日买入，当日卖出，賺取当日波段振幅价差！</div>
                  </div>
                </div>
                <div class="mid">
                  <div class="item">
                  ...
                  <div class="item">
                    ...
                    <div class="msg">如该交易未於收盘前卖出，则该交易於收盘后记为留仓单，收取该股收盘价 * 留仓股数 * {{sitesetInfo.stayFee}} 留仓费，（节假日亦收取）</div>
                  </div>
                </div>
                <div class="right">
                  <div class="item">
...
<script>
  ...
  export default {
    ...
    mounted () {
      this.getSettingSite()
      this.getInfoSite()
      this.$store.state.activeIndex = 'product'
    },
    methods: {
      async getInfoSite () {
        // 获取站点详情
      ...
      async getSettingSite () {
        // 获取站点设置信息
        let result = await api.getSetting()
        if (result.status === 0) {
          this.sitesetInfo = result.data
        } else {
          this.$message.error(result.msg)
        }
      }
...
<style lang="less" scoped>
  ...
  .panel-box {
    ...
    margin-top: 35px;
    width: 900px;
    box-shadow: 0.8px 0.6px 15px rgba(0, 0, 0, 0.1);
